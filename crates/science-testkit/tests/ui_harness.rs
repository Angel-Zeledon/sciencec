...
//!
//! Every test builds a throwaway directory of `.science` cases, runs the harness
...
use science_testkit::{run_ui_tests_with, UiTestOptions};
...
fn a_case_whose_expectation_matches_passes() {
    ...
    write(dir.join("case.science"), "fn main():\n    bad\n");
    write(dir.join("case.stderr"), "error[SC0100]: boom\n");

    let report =
        run_ui_tests_with(&dir, UiTestOptions::default(), constant("error[SC0100]: boom\n"));
...
fn a_case_whose_expectation_differs_fails() {
    ...
    write(dir.join("case.science"), "fn main():\n    bad\n");
    write(dir.join("case.stderr"), "error[SC0100]: expected this\n");

    let report =
        run_ui_tests_with(&dir, UiTestOptions::default(), constant("error[SC0100]: got that\n"));
    ...
    assert_eq!(read(dir.join("case.stderr")), "error[SC0100]: expected this\n");
}
...
fn a_missing_expectation_is_written_instead_of_failing() {
    ...
    let report =
        run_ui_tests_with(&dir, UiTestOptions::default(), constant("error[SC0003]: fresh\n"));

    assert_eq!(report.blessed(), 1);
    assert_eq!(report.failed(), 0);
    assert_eq!(report.passed(), 0);
    assert!(report.is_ok(), "a blessed case is not a failure");
    assert_eq!(read(dir.join("case.stderr")), "error[SC0003]: fresh\n");
}
...
fn crlf_in_the_expectation_file_is_normalised_away() {
    ...
    write(dir.join("case.stderr"), "error[SC0003]: tab\r\n  --> case.science:1:1\r\n");

    let report = run_ui_tests_with(
        &dir,
        UiTestOptions::default(),
        constant("error[SC0003]: tab\n  --> case.science:1:1\n"),
    );
...
fn cases_are_visited_in_a_stable_sorted_order() {
    ...
    for name in ["c", "a", "b"] {
        write(dir.join(format!("{name}.science")), "x\n");
    ...
    let names: Vec<String> = report
        ...
        .collect();
    assert_eq!(names, vec!["a.science", "b.science", "c.science"]);
}
...
fn nested_directories_are_walked() {
    ...
    write(dir.join("regions").join("deep.science"), "x\n");
...
fn the_compile_closure_receives_the_path_and_the_source_text() {
    ...
    write(dir.join("case.science"), "fn main():\n    bad\n");
    write(dir.join("case.stderr"), "case.science|fn main():\n    bad\n");
...
fn the_summary_counts_passes_failures_and_blessings() {
    ...
    assert!(summary.contains("SCIENCE_BLESS=1"), "{summary}");
}
