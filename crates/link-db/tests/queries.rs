...
//! the `SourceMap` that `link-diagnostics` renders against.

use link_db::{ast, file_diagnostics, source_text, tokens, Db, LinkDatabase};
use link_diagnostics::{FileId, Severity};
...
fn source_text_round_trips() {
    let mut db = LinkDatabase::new();
    let a = db.add_file("a.link", VALID);
    assert_eq!(source_text(&db, a), VALID);
    assert_eq!(db.path(a), "a.link");
...
fn a_path_keeps_one_file_id() {
    let mut db = LinkDatabase::new();
    let a = db.add_file("a.link", VALID);
    let b = db.add_file("b.link", VALID);
    assert_ne!(a, b);
    assert_eq!(db.add_file("a.link", "fn other():\n    2\n"), a);
    assert_eq!(db.file_id("a.link"), a);
...
fn removing_a_file_keeps_its_slot() {
    ...
    assert_eq!(db.add_file("a.link", VALID), a);
...
fn tokens_carry_the_lexers_diagnostics() {
    let mut db = LinkDatabase::new();
    // `§` is not a character of the language: LK0001.
    let a = db.add_file("a.link", "fn main():\n    §\n");
    ...
    assert_eq!(result.diagnostics()[0].code.to_string(), "LK0001");
...
fn ast_carries_the_parsers_diagnostics() {
    ...
    assert!(result
        .diagnostics()
        .iter()
        .all(|d| d.code.to_string().starts_with("LK01")));
}
...
fn a_diagnostic_from_a_query_renders_against_the_databases_source_map() {
    let mut db = LinkDatabase::new();
    let _pad = db.add_file("pad.link", VALID);
    let a = db.add_file("a.link", "fn main():\n    §\n");

    let diagnostic = tokens(&db, a).diagnostics()[0].clone();
    let map = db.source_map();
    let rendered = link_diagnostics::render(&map, &diagnostic);
