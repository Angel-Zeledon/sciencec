...
//! salsa's own `WillExecute` event, collected by [`LinkDatabase::with_execution_log`],
...
fn a_query_runs_once_and_is_then_reused() {
    let mut db = LinkDatabase::with_execution_log();
    let a = db.add_file("a.link", A);
...
fn re_adding_a_file_with_identical_text_invalidates_nothing() {
    ...
    db.clear_execution_log();
    let again = db.add_file("a.link", A);
