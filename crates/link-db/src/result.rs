...
use link_diagnostics::{Diagnostic, Diagnostics, Severity};

/// What a phase produced, together with what it complained about.
///
/// Every phase of Link recovers from errors instead of aborting (see the
/// `link-lexer` and `link-parser` crate docs), so "the result" and "the
