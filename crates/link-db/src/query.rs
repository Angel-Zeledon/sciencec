...
use link_diagnostics::Diagnostic;
...
/// The token stream of a file, with the lexical diagnostics (LK0001-LK0099)
...
pub fn tokens(db: &dyn Db, file: SourceFile) -> WithDiagnostics<Vec<Token>> {
    let (tokens, diagnostics) = link_lexer::lex(file.file_id(db), file.text(db));
    WithDiagnostics::new(tokens, diagnostics)
}

/// The syntax tree of a file, with the syntax diagnostics
/// (LK0100-LK0199) produced while building it.
///
...
pub fn ast(db: &dyn Db, file: SourceFile) -> WithDiagnostics<Module> {
    ...
    let (module, diagnostics) = link_parser::parse_module(tokens.value(), file.file_id(db));
