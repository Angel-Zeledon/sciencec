//! The compiler database: every phase of Link expressed as a memoized query.
//!
...
//!
//! ```
//! use link_db::{ast, tokens, LinkDatabase};
//!
//! let mut db = LinkDatabase::new();
//! let file = db.add_file("main.link", "fn main():\n    1\n");
//!
...
//!   it. See [`LinkDatabase::with_execution_log`].
//!
...
//! [`LinkDatabase::set_file_text`] filters that out, which is what makes §11
...
//! value it can turn into an id, which a bare `link_diagnostics::FileId` is
...
//! what [`link_diagnostics::SourceMap`] indexes — and
//! [`LinkDatabase::source_map`] guarantees the two agree.
...
pub use db::{Db, LinkDatabase};
pub use input::{SourceFile, Workspace};
pub use result::WithDiagnostics;

use link_diagnostics::{Diagnostic, FileId};
use link_lexer::Token;
use link_parser::ast::Module;
...
///
/// The tracked counterpart of [`LinkDatabase::present_files`]: a query that
