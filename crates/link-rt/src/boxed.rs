...
use crate::mem::{link_alloc, link_dealloc};
...
/// value of it.
#[no_mangle]
pub unsafe extern "C" fn link_box_new(info: *const LinkTypeInfo, value: *const u8) -> *mut u8 {
    // SAFETY: the caller guarantees a valid descriptor.
    let info = unsafe { &*info };
    // SAFETY: `info.align` is a power of two by the descriptor contract.
    let ptr = unsafe { link_alloc(info.size, info.align) };
...
/// [`link_box_new`] with that same descriptor and must not already have been
...
/// [`link_dealloc`](crate::link_dealloc) instead, so no destructor runs.
#[no_mangle]
pub unsafe extern "C" fn link_box_free(info: *const LinkTypeInfo, ptr: *mut u8) {
    // SAFETY: the caller guarantees a valid descriptor.
    let info = unsafe { &*info };
    ...
    // SAFETY: `ptr` came from `link_alloc` with exactly this size and
    // alignment.
    unsafe { link_dealloc(ptr, info.size, info.align) }
}
