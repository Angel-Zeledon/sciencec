...
//! Link program ever wants those, they belong in the language, not hidden here
//! where the ownership rules cannot see them.
//!
//! # Zero-sized allocations
//!
//! `size == 0` is legal everywhere in this module and never reaches the system
//! allocator. [`link_alloc`] answers with a *dangling* pointer — the integer
...
//! [`link_dealloc`] with `size == 0` does nothing. That makes the contract
...
#[inline(never)]
fn bad_align() -> ! {
    let message = b"link-rt: allocation alignment is not a power of two";
...
#[inline(never)]
fn bad_size() -> ! {
    let message = b"link-rt: allocation size exceeds the address space";
    // SAFETY: as above.
    unsafe { link_panic_bytes(message.as_ptr(), message.len()) }
}
...
pub(crate) fn allocation_failed() -> ! {
    let message = b"link-rt: out of memory";
    // SAFETY: as above.
    unsafe { link_panic_bytes(message.as_ptr(), message.len()) }
}
...
pub(crate) fn capacity_overflow() -> ! {
    let message = b"link-rt: capacity overflow";
    // SAFETY: as above.
    unsafe { link_panic_bytes(message.as_ptr(), message.len()) }
}
...
/// process aborts with a message rather than returning null: F0 gives a Link
...
#[no_mangle]
pub unsafe extern "C" fn link_alloc(size: usize, align: usize) -> *mut u8 {
    if size == 0 {
...
/// # Safety
///
/// `ptr` must come from [`link_alloc`] or [`link_realloc`] with exactly
/// `old_size` and `align`, and must not have been freed. `align` must be a
/// power of two.
#[no_mangle]
pub unsafe extern "C" fn link_realloc(
    ptr: *mut u8,
    old_size: usize,
    new_size: usize,
    align: usize,
...
///
/// `size == 0` is a no-op, matching [`link_alloc`].
///
/// # Safety
///
/// `ptr` must come from [`link_alloc`] or [`link_realloc`] with exactly `size`
/// and `align`, and must not already have been freed.
#[no_mangle]
pub unsafe extern "C" fn link_dealloc(ptr: *mut u8, size: usize, align: usize) {
    if size == 0 {
        return;
    }
    let layout = layout_of(size, align);
