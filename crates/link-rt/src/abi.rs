...
/// Destructor for one value of some Link type.
///
...
///
/// A `None` in a [`LinkTypeInfo`] means the type owns nothing and needs no
...
pub type LinkDropFn = unsafe extern "C" fn(value: *mut u8);

/// Hash one key, for [`LinkMap`](crate::LinkMap).
///
...
pub type LinkHashFn = unsafe extern "C" fn(key: *const u8) -> u64;

/// Compare two keys for equality, for [`LinkMap`](crate::LinkMap).
///
...
pub type LinkEqFn = unsafe extern "C" fn(a: *const u8, b: *const u8) -> bool;

/// Everything the runtime needs to know about a Link type it is storing.
///
/// Codegen emits one `static LinkTypeInfo` per monomorphised element type and
...
pub struct LinkTypeInfo {
    /// Size of one value, in bytes. May be zero: Link's `()` is zero-sized and
    /// `Array[()]` is legal. This is the stride as well as the size; Link, like
...
#[repr(C)]
#[derive(Clone, Copy, Debug)]
pub struct LinkMapInfo {
    /// Descriptor for the key type.
    pub key: LinkTypeInfo,
    /// Descriptor for the value type.
    pub value: LinkTypeInfo,
    /// Hash function for the key type.
    pub hash_fn: LinkHashFn,
