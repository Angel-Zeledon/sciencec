...
use crate::panic::link_panic_bytes;
...
/// [`link_string_free`], which codegen calls from drop glue when the ownership
/// rules say the value dies.
#[repr(C)]
#[derive(Clone, Copy)]
pub struct LinkString {
...
impl LinkString {
    ...
    /// # Safety
    ///
    /// `self` must be a live `LinkString`.
    #[inline]
    pub(crate) unsafe fn bytes(&self) -> &[u8] {
        // SAFETY: a live `LinkString` owns `len` initialised bytes at a
    ...
    /// # Safety
    ///
    /// `self` must be a live `LinkString`, whose bytes are UTF-8 by invariant.
    #[inline]
    ...
    pub(crate) unsafe fn from_raw_utf8(src: *const u8, len: usize) -> LinkString {
        if len == 0 {
            return LinkString::empty();
        }
        // SAFETY: `len` is non-zero, so this allocates `len` bytes at align 1.
        let ptr = unsafe { crate::mem::link_alloc(len, 1) };
    ...
    unsafe fn reserve(&mut self, additional: usize) {
        ...
        self.ptr = unsafe { link_realloc(self.ptr, self.cap, new_cap, 1) };
        self.cap = new_cap;
    }
}
...
#[no_mangle]
pub extern "C" fn link_string_new() -> LinkString {
    LinkString::empty()
}
...
/// invariant as given — [`link_string_truncate`] in particular would then be
...
#[no_mangle]
pub unsafe extern "C" fn link_string_from_bytes(ptr: *const u8, len: usize) -> LinkString {
    // SAFETY: the caller guarantees `len` readable bytes at `ptr`.
    let bytes = unsafe { std::slice::from_raw_parts(ptr, len) };
    if std::str::from_utf8(bytes).is_err() {
        let message = b"link-rt: String constructed from bytes that are not valid UTF-8";
        // SAFETY: a static byte string is a valid buffer of that length.
        unsafe { link_panic_bytes(message.as_ptr(), message.len()) }
    }
    // SAFETY: just validated.
    unsafe { LinkString::from_raw_utf8(ptr, len) }
}
...
/// # Safety
///
/// `value` must be a non-null, aligned pointer to a live [`LinkString`] whose
/// buffer has not already been freed.
#[no_mangle]
pub unsafe extern "C" fn link_string_free(value: *mut LinkString) {
    // SAFETY: the caller guarantees a live, uniquely borrowed `LinkString`.
    let value = unsafe { &mut *value };
    // SAFETY: `ptr`/`cap` describe the block this string allocated at align 1.
    unsafe { link_dealloc(value.ptr, value.cap, 1) };
    *value = LinkString::empty();
}
...
/// # Safety
///
/// `value` must be a non-null, aligned pointer to a live [`LinkString`].
#[no_mangle]
pub unsafe extern "C" fn link_string_clone(value: *const LinkString) -> LinkString {
    // SAFETY: the caller guarantees a live `LinkString`.
    let value = unsafe { &*value };
    // SAFETY: a live string's bytes are readable and valid UTF-8.
    unsafe { LinkString::from_raw_utf8(value.ptr, value.len) }
}
...
/// `value` must be a non-null, aligned pointer to a live [`LinkString`].
#[no_mangle]
pub unsafe extern "C" fn link_string_len(value: *const LinkString) -> i64 {
    // SAFETY: the caller guarantees a live `LinkString`.
...
/// `value` must be a non-null, aligned pointer to a live [`LinkString`].
#[no_mangle]
pub unsafe extern "C" fn link_string_is_empty(value: *const LinkString) -> bool {
    // SAFETY: the caller guarantees a live `LinkString`.
    unsafe { (*value).len == 0 }
}
...
///
/// **Codegen support**, for passing a Link string to foreign code and for
...
/// `value` must be a non-null, aligned pointer to a live [`LinkString`].
#[no_mangle]
pub unsafe extern "C" fn link_string_as_ptr(value: *const LinkString) -> *const u8 {
    // SAFETY: the caller guarantees a live `LinkString`.
    unsafe { (*value).ptr }
}
...
#[no_mangle]
pub unsafe extern "C" fn link_string_push_str(value: *mut LinkString, other: *const LinkString) {
...
/// `value` must be a non-null, aligned pointer to a live [`LinkString`].
#[no_mangle]
pub unsafe extern "C" fn link_string_truncate(value: *const LinkString, limit: i64) -> LinkString {
    if limit <= 0 {
        return LinkString::empty();
    }
    // SAFETY: the caller guarantees a live `LinkString`.
    ...
    unsafe { LinkString::from_raw_utf8(value.ptr, end) }
}
...
#[no_mangle]
pub unsafe extern "C" fn link_string_starts_with(
    value: *const LinkString,
    prefix: *const LinkString,
) -> bool {
...
/// # Safety
///
/// `value` must be a non-null, aligned pointer to a live [`LinkString`], and it
/// must stay alive and unmodified for as long as the returned [`LinkChars`] is
/// used. The region engine guarantees that; this function does not check it.
#[no_mangle]
pub unsafe extern "C" fn link_string_chars(value: *const LinkString) -> LinkChars {
    // SAFETY: the caller guarantees a live `LinkString`.
    let value = unsafe { &*value };
    LinkChars {
        ptr: value.ptr,
...
/// Advance a [`LinkChars`], yielding `Option[Char]`.
///
...
/// # Safety
///
/// `iter` must be a non-null, aligned pointer to a live [`LinkChars`] whose
/// string is still alive; `out` must be non-null, aligned for `u32`, and
/// writable.
#[no_mangle]
pub unsafe extern "C" fn link_chars_next(iter: *mut LinkChars, out: *mut u32) -> bool {
    // SAFETY: the caller guarantees a live, uniquely borrowed `LinkChars`.
...
/// Both pointers must be non-null, aligned and point to live [`LinkString`]s.
#[no_mangle]
pub unsafe extern "C" fn link_string_eq(a: *const LinkString, b: *const LinkString) -> bool {
    // SAFETY: the caller guarantees two live strings.
...
/// Both pointers must be non-null, aligned and point to live [`LinkString`]s.
#[no_mangle]
pub unsafe extern "C" fn link_string_cmp(a: *const LinkString, b: *const LinkString) -> i32 {
    // SAFETY: the caller guarantees two live strings.
    let (a, b) = unsafe { ((*a).bytes(), (*b).bytes()) };
    match a.cmp(b) {
...
/// choosing keys; when Link grows a story for hostile input, that story
/// replaces this function and nothing else.
///
/// Equal strings hash equal, which is the one property [`LinkMapInfo`] requires.
///
/// [`LinkMapInfo`]: crate::LinkMapInfo
///
/// # Safety
///
/// `value` must be a non-null, aligned pointer to a live [`LinkString`].
#[no_mangle]
pub unsafe extern "C" fn link_string_hash(value: *const LinkString) -> u64 {
