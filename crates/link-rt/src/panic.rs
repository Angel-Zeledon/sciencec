...
//! is emitted anywhere in a Link binary.
//!
...
//! and it would have to be built before the first Link program ever ran.
//!
...
//! would be guessing.
//!
//! Standard output is flushed before the abort. Link's stdout is line
...
#[no_mangle]
pub extern "C" fn link_abort() -> ! {
    flush_stdout();
    std::process::abort()
}

/// Link's `panic(message: &String) -> Never`.
///
...
///
/// `Never` (§4.5) is the type of this call in Link, and it coerces to any type,
/// which is what lets a `match` arm panic.
///
/// # Safety
///
/// `message` must be a non-null, aligned pointer to a live [`LinkString`].
#[no_mangle]
pub unsafe extern "C" fn link_panic(message: *const LinkString) -> ! {
    // SAFETY: the caller guarantees `message` points to a live `LinkString`.
    let text = unsafe { &*message };
    // SAFETY: a live `LinkString` owns `len` initialised bytes at `ptr`.
    unsafe { link_panic_bytes(text.ptr, text.len) }
}
...
#[no_mangle]
pub unsafe extern "C" fn link_panic_bytes(ptr: *const u8, len: usize) -> ! {
