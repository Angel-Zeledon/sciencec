...
//! gives them, so that `science-resolve`, `science-types`, `science-mir`,
//! `science-regions` and `science-codegen` plug into a shape that is already fixed
...
/// A module: one file, or one directory with a `mod.science` (§12 of the spec —
/// F0 has no `mod` declaration, so a module is a file).
///
/// Placeholder. `science-resolve` owns the real one.
...
/// LLVM module.
///
/// Placeholder. `science-codegen` owns the real one.
#[salsa::interned(unsafe(no_lifetime), revisions = usize::MAX, debug)]
...
pub fn module_tree(db: &dyn Db) -> ModuleTree {
    let _ = db;
    unimplemented!("science-resolve: AST -> module graph")
}
...
pub fn hir(db: &dyn Db, module: ModuleId) -> Hir {
    let _ = (db, module);
    unimplemented!("science-resolve: AST -> HIR")
}
...
pub fn signature(db: &dyn Db, def: DefId) -> Signature {
    let _ = (db, def);
    unimplemented!("science-types: signature of a definition")
}
...
pub fn type_of(db: &dyn Db, def: DefId) -> Type {
    let _ = (db, def);
    unimplemented!("science-types: type of a definition")
}
...
pub fn thir(db: &dyn Db, def: DefId) -> Thir {
    let _ = (db, def);
    unimplemented!("science-types: HIR -> THIR")
}
...
pub fn mir(db: &dyn Db, def: DefId) -> Mir {
    let _ = (db, def);
    unimplemented!("science-mir: THIR -> MIR")
}
...
/// the shape is deliberate.** §6.2 of the design spec: a Science signature carries
...
pub fn region_result(db: &dyn Db, def: DefId) -> RegionResult {
    let _ = (db, def);
    unimplemented!("science-regions: region inference over the call graph")
}
...
pub fn mono_items(db: &dyn Db) -> Vec<MonoItem> {
    let _ = db;
    unimplemented!("science-codegen: monomorphization")
}
...
pub fn llvm_module(db: &dyn Db, unit: CodegenUnit) -> LlvmIr {
    let _ = (db, unit);
    unimplemented!("science-codegen: LLVM IR emission via inkwell")
}
