...
/// the later crates (`science-resolve`, `science-types`, ...) can define their own
/// queries without depending on `ScienceDatabase`'s private state.
///
...
pub struct ScienceDatabase {
    ...
    /// is enabled. See [`ScienceDatabase::with_execution_log`].
    log: Option<Log>,
}

impl Default for ScienceDatabase {
    fn default() -> Self {
        Self::new()
    }
}

impl ScienceDatabase {
    /// An empty database.
    pub fn new() -> Self {
        Self::build(None)
    }
    ...
    ///
    /// [`add_file`]: ScienceDatabase::add_file
    ...
    /// reason as [`ScienceDatabase::set_file_text`].
    ...
    // --- the bridge to science-diagnostics -----------------------------------
    ...
    /// [`ScienceDatabase::with_execution_log`].
...
#[salsa::db]
impl salsa::Database for ScienceDatabase {}

#[salsa::db]
impl Db for ScienceDatabase {
    fn try_source_file(&self, file: FileId) -> Option<SourceFile> {
