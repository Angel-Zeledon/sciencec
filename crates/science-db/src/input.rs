...
//! exclusively through the methods on [`crate::ScienceDatabase`], which is what
...
/// # Invariants
///
/// These hold because `ScienceDatabase` is the only thing that creates or writes
/// a `SourceFile`, and a lot depends on them:
///
/// 1. `file_id` is the slot this input was created in and never changes. It is
///    the same `FileId` a [`science_diagnostics::SourceMap`] built from this
