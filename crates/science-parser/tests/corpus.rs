...
fn check(path: &Path) -> String {
    ...
    let (tokens, lex_diagnostics) = science_lexer::lex(FileId(0), &source);
    let (_module, parse_diagnostics) = science_parser::parse_module(&tokens, FileId(0));
...
fn every_example_produces_items() {
    for path in corpus() {
        ...
        let (module, _) = science_parser::parse_module(&tokens, FileId(0));
