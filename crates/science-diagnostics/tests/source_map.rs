...
use science_diagnostics::source_map::{LineCol, SourceMap};
...
fn map_of(text: &str) -> (SourceMap, FileId) {
    ...
    let f = m.add_file("test.science".into(), text.into());
    (m, f)
}
...
fn each_file_gets_its_own_id() {
    let mut m = SourceMap::new();
    let a = m.add_file("a.science".into(), "one".into());
    let b = m.add_file("b.science".into(), "two".into());
    assert_ne!(a, b);
    assert_eq!(m.path(a), "a.science");
    assert_eq!(m.path(b), "b.science");
