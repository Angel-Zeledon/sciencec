...
use science_diagnostics::render::{render, render_all};
use science_diagnostics::source_map::SourceMap;
...
fn full_diagnostic_matches_the_reference_layout() {
    ...
    let f = map.add_file("src/main.science".into(), MOVED.into());
...
fn an_empty_span_gets_a_single_caret() {
    ...
    let expected = "\
error[SC0110]: expected `:` after the signature
 --> t.science:1:10
  |
1 | fn main()
  |          ^ insert `:` here";
...
fn distant_lines_are_elided_and_a_single_gap_is_printed() {
    ...
    let f = map.add_file("wide.science".into(), text.clone());
...
fn labels_in_another_file_get_their_own_header() {
    let mut map = SourceMap::new();
    let a = map.add_file("a.science".into(), "fn helper(x: Int):\n    x\n".into());
    let b = map.add_file("b.science".into(), "fn main():\n    helper(\"hi\")\n".into());
...
fn render_all_orders_by_file_and_position() {
    let mut map = SourceMap::new();
    let a = map.add_file("a.science".into(), "one\ntwo\nthree\n".into());
    let b = map.add_file("b.science".into(), "alpha\nbeta\n".into());
