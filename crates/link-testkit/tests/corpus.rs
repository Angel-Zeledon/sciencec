...
//!
//! The `.link` corpus is input for other crates' test suites, and a mistake
...
use link_testkit::{expectation_path, normalize};

fn repository_root() -> PathBuf {
    Path::new(env!("CARGO_MANIFEST_DIR")).join("..").join("..")
}

fn link_files(dir: &Path) -> Vec<PathBuf> {
    ...
    for entry in entries.flatten() {
        let path = entry.path();
        if path.is_dir() {
            found.extend(link_files(&path));
        } else if path.extension().and_then(|e| e.to_str()) == Some("link") {
            found.push(path);
        }
    }
    found.sort();
...
fn no_example_uses_a_tab_anywhere() {
    // §4.1: indentation is spaces only, and a tab in it is LK0003. Every
    ...
    // track of.
    for path in link_files(&repository_root().join("examples")) {
...
fn every_example_is_listed_in_the_readme() {
    ...
    for path in link_files(&examples) {
...
fn the_acceptance_program_exists_and_names_every_requirement() {
    let path = repository_root().join("examples").join("00_kitchen_sink.link");
...
fn every_ui_case_has_an_expectation_next_to_it() {
    ...
    for case in cases {
        ...
        assert!(
            expectation.is_file(),
            "{} has no expectation; add one or bless it with LINK_BLESS=1",
            case.display()
        );
    }
}
