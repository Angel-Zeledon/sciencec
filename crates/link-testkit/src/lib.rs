//! UI tests for `linkc`, in the style of rustc's.
//!
//! A UI test is a `.link` program that must be rejected, sitting next to a
...
pub const SOURCE_EXTENSION: &str = "link";
...
pub const BLESS_ENV_VAR: &str = "LINK_BLESS";
...
impl UiTestOptions {
    ...
    /// `LINK_BLESS=0` means what it looks like it means.
...
///
/// `compile` is handed the path of the `.link` file and its text, and returns
...
/// The `.stderr` file that goes with a `.link` source.
...
fn run_one(
    ...
    // The source is normalised too, so that a `.link` file checked out with
...
/// Collect every `.link` file under `dir`, depth first, sorted by name.
///
...
mod tests {
    ...
    fn the_expectation_sits_next_to_the_source() {
        let expectation = expectation_path(Path::new("tests/ui/tab_in_indentation.link"));
