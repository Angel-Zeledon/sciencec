...
fn every_example_lexes_without_a_single_diagnostic() {
    let files = link_files(&examples_dir());
    ...
    for path in &files {
        ...
        if !diagnostics.is_empty() {
            report.push_str(&link_diagnostics::render_all(&map, &diagnostics));
            report.push('\n');
        }
    }
...
fn every_example_ends_in_a_dedent_to_column_zero() {
    ...
    for path in link_files(&examples_dir()) {
        let source = std::fs::read_to_string(&path).unwrap().replace("\r\n", "\n");
        let (tokens, _) = link_lexer::lex(link_diagnostics::FileId(0), &source);
        assert_eq!(
            tokens.last().map(|t| &t.kind),
            Some(&link_lexer::TokenKind::Eof),
            "{} does not end in Eof",
            path.display()
        );
    }
