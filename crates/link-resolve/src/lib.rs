//! Link's name resolution: AST in, HIR out.
//!
...
//! - [`dump`] renders an HIR tree the way `link-parser` renders an AST.
//!
//! ```no_run
...
//! let (krate, diagnostics) = link_resolve::resolve_module(FileId(0), "main.link", &ast);
//! print!("{}", link_resolve::dump::dump_crate(&krate));
...
///
/// §9 of the design spec gives `LK0200`-`LK0299` to "resolution and types",
/// which is two phases sharing one range. The split, agreed with the phase
/// that owns the other half:
///
/// | Range | Owner |
/// |---|---|
/// | `LK0200`-`LK0249` | `link-resolve`: names, scopes, modules, imports, the orphan rule |
/// | `LK0250`-`LK0299` | `link-types`: inference, trait bounds, exhaustiveness |
///
/// With one exception, which is in the spec rather than in this split: §4.4
/// names `LK0210` for a non-exhaustive `match`. That is an exhaustiveness
/// check and so belongs to `link-types`, on the wrong side of the line. The
/// code is therefore **left unassigned here**, reserved for `link-types` to
...
pub mod codes {
    ...
    // `match`, which `link-types` reports. See the module documentation.
    ...
    mod tests {
        ...
        fn every_code_is_this_crates_to_emit() {
            for code in ALL {
                assert!(
                    ...
                    "{code} is outside LK0200-LK0249, which is this crate's range"
                );
            }
        }
        ...
        fn the_code_section_4_4_reserves_for_exhaustiveness_is_left_alone() {
            assert!(
                ...
                "LK0210 belongs to the type checker's exhaustiveness check (§4.4)"
            );
        }
    }
}
