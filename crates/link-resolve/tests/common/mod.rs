...
pub fn report_crate(files: Vec<(&str, Module)>) -> String {
    let sources: Vec<link_resolve::SourceModule> = files
        .into_iter()
        .enumerate()
        .map(|(index, (path, ast))| link_resolve::SourceModule {
            file: FileId(index as u32),
            path: path.to_string(),
            ast,
        })
        .collect();
    let (krate, diagnostics) = link_resolve::resolve_crate(&sources);
    render(&krate, &diagnostics)
}

pub fn render(krate: &link_resolve::hir::Crate, diagnostics: &Diagnostics) -> String {
    let mut out = link_resolve::dump::dump_crate(krate);
...
pub fn diagnose(module: &Module) -> Diagnostics {
    link_resolve::resolve_module(FILE, "main.link", module).1
}
