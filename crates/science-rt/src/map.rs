...
//! any iteration order, and §8 gives Science no way to observe one.

use crate::abi::ScienceMapInfo;
...
impl ScienceMap {
    ...
    #[inline]
    unsafe fn key_slot(&self, info: &ScienceMapInfo, index: usize) -> *mut u8 {
    ...
    /// `index` must be less than `cap`, and `info` must describe this table.
    #[inline]
    unsafe fn value_slot(&self, info: &ScienceMapInfo, index: usize) -> *mut u8 {
    ...
    unsafe fn find(&self, info: &ScienceMapInfo, key: *const u8) -> Option<usize> {
        if self.cap == 0 {
            return None;
        }
        let mask = self.cap - 1;
    ...
    /// key must be absent.
    unsafe fn insert_fresh(&mut self, info: &ScienceMapInfo, key: *const u8, value: *const u8) {
        let mask = self.cap - 1;
    ...
    unsafe fn rehash(&mut self, info: &ScienceMapInfo, new_cap: usize) {
        ...
        unsafe {
            science_dealloc(old.states, old.cap, 1);
            science_dealloc(old.keys, info.key.offset_of(old.cap), info.key.align);
            science_dealloc(old.values, info.value.offset_of(old.cap), info.value.align);
        }
    }
...
/// value was moved out by [`science_map_remove`] are not destroyed, since they are
/// no longer the map's.
///
...
pub unsafe extern "C" fn science_map_free(map: *mut ScienceMap, info: *const ScienceMapInfo) {
    ...
    // SAFETY: the three blocks came from `science_alloc` with exactly these sizes
    // and alignments.
    unsafe {
        science_dealloc(map.states, map.cap, 1);
        science_dealloc(map.keys, info.key.offset_of(map.cap), info.key.align);
        science_dealloc(map.values, info.value.offset_of(map.cap), info.value.align);
    }
