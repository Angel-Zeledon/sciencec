...
fn box_new_moves_the_value_onto_the_heap() {
    unsafe {
        let info = i64_info();
        let value = 1234i64;
        let b = science_box_new(&info, (&raw const value).cast::<u8>());
        assert!(
            !b.is_null(),
...
fn box_of_a_zero_sized_type_allocates_nothing_but_is_still_non_null() {
    unsafe {
        let info = unit_info();
        let unit: [u8; 0] = [];
        let b = science_box_new(&info, unit.as_ptr());
        assert!(!b.is_null());
        science_box_free(&info, b);
    }
}
...
fn box_of_a_string_owns_the_string() {
    unsafe {
        ...
        let value = s("boxed");
        let b = science_box_new(&info, (&raw const value).cast::<u8>());
        assert_eq!(as_str(&*b.cast::<ScienceString>()), "boxed");
...
fn many_boxes_are_distinct_allocations() {
    unsafe {
        ...
        for i in 0..256i64 {
            boxes.push(science_box_new(&info, (&raw const i).cast::<u8>()));
        }
