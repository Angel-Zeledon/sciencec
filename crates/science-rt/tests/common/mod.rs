...
pub fn s(text: &str) -> ScienceString {
    unsafe { science_string_from_bytes(text.as_ptr(), text.len()) }
}

/// Borrow a `String`'s bytes as a Rust `&str`.
pub fn as_str(value: &ScienceString) -> &str {
    unsafe {
...
pub fn free(mut value: ScienceString) {
    unsafe { science_string_free(&mut value) }
}
...
pub fn counted_info() -> ScienceTypeInfo {
    ScienceTypeInfo {
...
pub fn i64_info() -> ScienceTypeInfo {
    ScienceTypeInfo {
        size: 8,
        align: 8,
        drop_fn: None,
...
/// A zero-sized element type, the representation of Science's `()`.
pub fn unit_info() -> ScienceTypeInfo {
    ScienceTypeInfo {
        size: 0,
        align: 1,
        drop_fn: None,
...
pub fn over_info() -> ScienceTypeInfo {
    ScienceTypeInfo {
...
pub extern "C" fn hash_link_string(key: *const u8) -> u64 {
    unsafe { science_string_hash(key.cast::<ScienceString>()) }
}

pub extern "C" fn eq_link_string(a: *const u8, b: *const u8) -> bool {
    unsafe { science_string_eq(a.cast::<ScienceString>(), b.cast::<ScienceString>()) }
}

pub extern "C" fn drop_link_string(value: *mut u8) {
    unsafe { science_string_free(value.cast::<ScienceString>()) }
}

pub fn string_info() -> ScienceTypeInfo {
    ScienceTypeInfo {
        size: std::mem::size_of::<ScienceString>(),
        align: std::mem::align_of::<ScienceString>(),
        drop_fn: Some(drop_link_string),
    }
}

pub fn u64_map_info() -> ScienceMapInfo {
    ScienceMapInfo {
        key: i64_info(),
        value: i64_info(),
        hash_fn: hash_u64,
...
pub fn colliding_map_info() -> ScienceMapInfo {
    ScienceMapInfo {
        key: i64_info(),
        value: i64_info(),
