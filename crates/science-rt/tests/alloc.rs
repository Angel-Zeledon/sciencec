...
fn alloc_honours_large_alignment() {
    unsafe {
        for align in [1usize, 2, 4, 8, 16, 32, 64, 128] {
            let p = science_alloc(align * 3, align);
            assert!(!p.is_null());
            assert_eq!(p as usize % align, 0, "alignment {align} not honoured");
            science_dealloc(p, align * 3, align);
        }
    }
}
...
fn realloc_grows_and_preserves_contents() {
    unsafe {
        let p = science_alloc(16, 8);
        for i in 0..16u8 {
            *p.add(i as usize) = i;
        }
        let p = science_realloc(p, 16, 4096, 8);
        assert_eq!(p as usize % 8, 0);
        for i in 0..16u8 {
            assert_eq!(*p.add(i as usize), i, "byte {i} lost across realloc");
        }
        science_dealloc(p, 4096, 8);
    }
}
...
fn realloc_shrinks_and_preserves_the_prefix() {
    unsafe {
        let p = science_alloc(256, 8);
        for i in 0..256usize {
            *p.add(i) = (i % 251) as u8;
        }
        let p = science_realloc(p, 256, 8, 8);
        for i in 0..8usize {
            assert_eq!(*p.add(i), (i % 251) as u8);
        }
        science_dealloc(p, 8, 8);
    }
}
...
fn realloc_from_zero_allocates() {
    unsafe {
        let p = science_alloc(0, 8);
        let p = science_realloc(p, 0, 32, 8);
        assert!(!p.is_null());
        std::ptr::write_bytes(p, 7, 32);
        assert_eq!(*p.add(31), 7);
        science_dealloc(p, 32, 8);
    }
}
...
fn realloc_to_zero_frees_and_returns_dangling() {
    unsafe {
        let p = science_alloc(32, 8);
        let p = science_realloc(p, 32, 0, 8);
        assert!(!p.is_null());
        assert_eq!(p as usize % 8, 0);
        science_dealloc(p, 0, 8);
    }
}
...
fn realloc_to_the_same_size_is_valid() {
    unsafe {
        let p = science_alloc(32, 8);
        *p = 9;
        let p = science_realloc(p, 32, 32, 8);
        assert_eq!(*p, 9);
        science_dealloc(p, 32, 8);
    }
}
